...
function IntegrationCard({
  ...
  return (
    <div
      className={`flex flex-col gap-2 border-2 border-accent bg-white/[0.03] p-6 transition-all duration-500 [@media(max-height:820px)]:p-3 ${
        revealed ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
      }`}
    >
      <h3 className="text-lg font-extrabold text-foreground">{card.title}</h3>
      <p className="text-sm leading-relaxed text-foreground-secondary">{card.desc}</p>
    </div>
  );
}
...
function IntegrationsContent({ progress }: { progress: MotionValue<number> }) {
  ...
  return (
    <div className="relative flex h-full flex-col bg-background px-6 pt-[100px] pb-10 md:px-16 [@media(max-height:820px)]:pt-[64px] [@media(max-height:820px)]:pb-6">
      <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col">
        <div className="flex flex-1 flex-col gap-8 border-2 border-white/15 bg-white/[0.03] p-8 md:p-12 [@media(max-height:820px)]:gap-2 [@media(max-height:820px)]:p-4">
          <div>
            <p className="text-xs font-semibold tracking-widest text-accent uppercase">
              אינטגרציות
            </p>
            <h2 className="mt-4 max-w-2xl text-3xl font-extrabold leading-tight text-foreground md:text-5xl [@media(max-height:820px)]:mt-1">
              מתחבר למערכות שכבר עובדות אצלך.
            </h2>
            <p className="mt-4 max-w-xl text-base leading-relaxed text-foreground-secondary [@media(max-height:820px)]:mt-1">
